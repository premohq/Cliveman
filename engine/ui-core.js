/* ========================================================================
   ...
   ======================================================================== */
/* Pause-aware delay used by story typing, transitions, and the lightweight
   minigames. The pause menu can interrupt an in-flight timer without letting
   the elapsed paused time count toward the delay. */
window._gamePaused=!!window._gamePaused;
window.setGamePaused=function(paused){
  paused=!!paused;
  if(window._gamePaused===paused)return;
  window._gamePaused=paused;
  if(paused&&window._fm)window._fm.held={};
  var ev;
  try{ev=new CustomEvent('clivepausechange',{detail:{paused:paused}});}
  catch(e){ev=document.createEvent('Event');ev.initEvent('clivepausechange',false,false);ev.detail={paused:paused};}
  document.dispatchEvent(ev);
};
function sleep(ms){
  return new Promise(function(resolve){
    var remaining=Math.max(0,Number(ms)||0),timer=null,started=0,done=false;
    function now(){return (typeof performance!=='undefined'&&performance.now)?performance.now():Date.now();}
    function cleanup(){document.removeEventListener('clivepausechange',onPauseChange);}
    function finish(){if(done)return;done=true;timer=null;cleanup();resolve();}
    function arm(){
      if(done||window._gamePaused)return;
      if(remaining<=0){finish();return;}
      started=now();
      timer=setTimeout(function(){timer=null;remaining=0;finish();},remaining);
    }
    function onPauseChange(){
      if(window._gamePaused){
        if(timer!==null){clearTimeout(timer);timer=null;remaining=Math.max(0,remaining-(now()-started));}
      }else arm();
    }
...
/* Speaker name plate: a 'Name: ...' speaker line shows the name on the
   ...
   Narration/system lines retire the plate. Full-screen nav is untouched
   (toasts and the hidden terminal keep the full line), while nav-dialog
   conversations share the normal plate. Voice is always given the
...
function dlgAdvanceGate(lineDiv){
  return new Promise(function(resolve){
    ...
    function pollPad(){
      if(done)return;
      if(window._gamePaused){padPrev=true;raf=requestAnimationFrame(pollPad);return;}
...
var DLG_SPK_RE=window.CLIVE_SPEAKER_RE||/^\s*([^:"：]{2,30})[:：]\s*/;
function setDlgName(text,cls){
  var el=document.getElementById('dlgName');
  if(!el||(document.body.classList.contains('nav-mode')&&!document.body.classList.contains('nav-dialog')))return text;
  if(cls==='speaker'){
...
window.navReadGate=function(){
  return new Promise(function(resolve){
    ...
    var _readTpl=window._padConnected?'PRESS {CONFIRM} TO CONTINUE':'PRESS ENTER TO CONTINUE';var _readText=window.t?window.t(_readTpl):_readTpl;_readText=_readText.replace(/\{CONFIRM\}/g,labelConfirm());prompt.textContent='\u25B6 '+_readText;
    ...
    function onKey(e){ if(window._gamePaused)return; if(e.key==='Enter'||e.key===' '){ e.preventDefault(); e.stopPropagation(); finish(); } }
    function onPromptClick(){if(!window._gamePaused)finish();}
    document.addEventListener('keydown',onKey,true);
    prompt.addEventListener('click',onPromptClick);
    function pollPad(){ if(done)return; if(window._gamePaused){padPrev=true;raf=requestAnimationFrame(pollPad);return;} var ps=navigator.getGamepads?navigator.getGamepads():[]; var gp=null; for(var i=0;i<ps.length;i++){ if(ps[i]){gp=ps[i];break;} } if(gp){ var b=gp.buttons||[]; var c=!!(b[0]&&b[0].pressed); if(c&&!padPrev){ finish(); return; } padPrev=c; } else padPrev=false; raf=requestAnimationFrame(pollPad); }
    raf=requestAnimationFrame(pollPad);
  });
};
