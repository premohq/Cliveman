/* ========================================================================
   ...
   ======================================================================== */
const state={inventory:[],checkpoint:0,roomId:0,savedRow:0,savedCol:0,called_bs_clemons:false,f2_badge:false,f3_poison:false,kw1_found:false,kw2_found:false,bevan_room_known:false,ch2_tnt:false,ch2_poison:false,resumeRoomId:-1,resumePos:null,money:0,mechanicPaid:false,enemiesBeat:0,level:1,secretEnding:false,heading:0,atDriveStart:false,};function encodeSave(){const cp=state.checkpoint&0x7;const room=state.roomId&0xF;const row=state.savedRow&0xF;const col=state.savedCol&0xF;let flags=0;if(state.called_bs_clemons)flags|=1;if(state.f2_badge)flags|=2;if(state.f3_poison)flags|=4;if(state.kw1_found)flags|=8;if(state.kw2_found)flags|=16;if(state.bevan_room_known)flags|=32;if(state.ch2_tnt)flags|=64;if(state.ch2_poison)flags|=128;state.money=Math.max(0,Math.min(1023,state.money));const low=cp|(room<<3)|(row<<7)|(col<<11)|(flags<<15);const money=state.money&0x3FF;const mech=state.mechanicPaid?1:0;const drive=state.atDriveStart?1:0;const eb=Math.max(0,Math.min(127,state.enemiesBeat|0));const high=money|(mech<<10)|(drive<<11)|(eb<<12);return low+high*0x800000;}function decodeSave(code){if(typeof code==='string'&&/^\d+$/.test(code))code=parseInt(code,10);if(typeof code!=='number'||!isFinite(code)||code<0||code>0x3FFFFFFFFFF)return null;const low=code%0x800000;const high=Math.floor(code/0x800000);const cp=low&0x7;const room=(low>>3)&0xF;const row=(low>>7)&0xF;const col=(low>>11)&0xF;const flags=(low>>15)&0xFF;const money=high&0x3FF;const mech=(high>>10)&0x1;const drive=(high>>11)&0x1;const eb=(high>>12)&0x7F;if(cp<1||cp>7)return null;return{cp:cp,room:room,row:row,col:col,flags:flags,money:money,mech:mech,drive:drive,eb:eb};}function restoreState(dec){state.checkpoint=dec.cp;state.roomId=dec.room;state.savedRow=dec.row;state.savedCol=dec.col;state.called_bs_clemons=!!(dec.flags&1);state.f2_badge=!!(dec.flags&2);state.f3_poison=!!(dec.flags&4);state.kw1_found=!!(dec.flags&8);state.kw2_found=!!(dec.flags&16);state.bevan_room_known=!!(dec.flags&32);state.ch2_tnt=!!(dec.flags&64);state.ch2_poison=!!(dec.flags&128);state.money=dec.money;state.mechanicPaid=!!dec.mech;state.atDriveStart=!!dec.drive;state.enemiesBeat=dec.eb||0;(function(){let lvl=1;let threshold=2;let e=state.enemiesBeat;while(e>=threshold){lvl++;threshold*=2;}state.level=lvl;})();state.resumeRoomId=dec.room;state.resumePos=[dec.row,dec.col];const inv=[];if(dec.cp>=2)inv.push("factory map");if(state.f2_badge)inv.push("factory access badge");if(state.f3_poison)inv.push("rat poison (burnt box)");if(dec.cp>=3)inv.push("apartment building map");if(state.ch2_tnt)inv.push("undetonated TNT");if(state.ch2_poison)inv.push("rat poison (crime scene)");state.inventory=inv;}window.showSaveCode = function showSaveCode(){
  const code=encodeSave();
  const payload={v:1,code:code,ts:Date.now(),game:'CLIVEMAN',ver:window.CLIVEMAN_VERSION};
  ...
  blank();
  /* All three save/load boxes are 46 columns wide (see story/title.js). */
  const box='  +==============================================+\n'
    +'  |   SAVE FILE WRITTEN                          |\n'
