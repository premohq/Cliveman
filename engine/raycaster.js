...
const rayAngle=angle-HALF_FOV+(r/NUM_RAYS)*FOV;
...
/* record perpDist across every screen column this ray covers (for sprite occlusion) */
for(let bx=0;bx<RAY_WIDTH&&screenX+bx<W;bx++){zBuffer[screenX+bx]=(dist>maxDist)?maxDist:perpDist;}
...
/* Draw the wall slice RAY_WIDTH pixels wide so the whole screen is covered
   even when we cast fewer rays than there are pixels (mobile). */
const drawW=(screenX+RAY_WIDTH<=W)?RAY_WIDTH:(W-screenX);
ctx.drawImage(tex,texX,0,1,TEX_SIZE,screenX,wallTop,drawW,wallHeight);
const fogAlpha=perpDist<10?perpDist/10:0.85;
ctx.fillStyle='rgba('+fogR+','+fogG+','+fogB+','+(fogAlpha>0.85?0.85:fogAlpha)+')';
ctx.fillRect(screenX,wallTop,drawW,wallHeight);
}
const sprites=[];
